 ...
 * request at a time (Lambda); on a concurrent runtime (ECS) it is a cross-request leak. The
 * field names and the method shapes behind them are the monorepo Context's porcelain
 * (`ctx.logger`, `ctx.metrics`, `ctx.requestId`), so handlers port without renames.
 *
