...
describe('OrderServiceFadesSource', () => {
  ...
  describe('buildFadeRows (V2_FADE_RATE_SQL semantics)', () => {
    ...
    it('emits one row per completed, resolved order in the SQL row shape', () => {
      ...
      expect(rows).toEqual([
        { fillerAddress: ADDR_A, faded: 1, postTimestamp: NOW - 200, deadline: NOW - 100, orderHash: a.orderHash },
      ]);
    });
    ...
    it('produces rows with exactly the keys and value types the Redshift repository returns', async () => {
      ...
      const SHAPE_ORDER_HASH = '0x1111111111111111111111111111111111111111111111111111111111111111';
      ...
      const [newRow] = buildFadeRows(
        [fade({ deadline: NOW - 100, postedAt: NOW - 200, orderHash: SHAPE_ORDER_HASH })],
        {
          now: NOW,
        }
      );
      ...
      expect(rows).toEqual([
        {
          fillerAddress: ADDR_A,
          faded: 0,
          postTimestamp: kept.postedAt,
          deadline: kept.deadline,
          orderHash: kept.orderHash,
        },
      ]);
    });
    ...
    it('latest-N is per address, not per filler identity', () => {
      ...
      expect(rows.filter((r) => r.fillerAddress === ADDR_A2)).toEqual([
        {
          fillerAddress: ADDR_A2,
          faded: 1,
          postTimestamp: a2Fade.postedAt,
          deadline: a2Fade.deadline,
          orderHash: a2Fade.orderHash,
        },
      ]);
    });
      ...
      expect(buildFadeRows([cancelled], { now: NOW, policy: { countNeverFilledTerminalAsFade: true } })).toEqual([
        {
          fillerAddress: ADDR_A,
          faded: 1,
          postTimestamp: cancelled.postedAt,
          deadline: cancelled.deadline,
          orderHash: cancelled.orderHash,
        },
      ]);
      ...
      expect(rows).toEqual([
        {
          fillerAddress: ADDR_A,
          faded: 0,
          postTimestamp: aFill.postedAt,
          deadline: aFill.deadline,
          orderHash: aFill.orderHash,
        },
        {
          fillerAddress: ADDR_A,
          faded: 1,
          postTimestamp: aFade.postedAt,
          deadline: aFade.deadline,
          orderHash: aFade.orderHash,
        },
        {
          fillerAddress: ADDR_B,
          faded: 1,
          postTimestamp: bFade.postedAt,
          deadline: bFade.deadline,
          orderHash: bFade.orderHash,
        },
      ]);
