...
const row = (fillerAddress: string, faded: 0 | 1, deadline: number): V2FadesRowType => ({
  ...
  deadline,
  orderHash: `0xorder${fillerAddress.slice(-2)}at${deadline}`,
});
...
const BLOCK_A_DECISIONS: ToUpdateTimestampRow[] = [
  {
    ...
    // the block entry names the faded orders that caused it
    fadedOrderHashes: BLOCKING_ROWS.filter((r) => r.fillerAddress === ADDR_A && r.faded === 1).map((r) => r.orderHash),
  },
  {
    hash: FILLER_B,
    lastExaminedTimestamp: NOW,
...
const ALL_CLEAN_DECISIONS: ToUpdateTimestampRow[] = BLOCK_A_DECISIONS.map((d) => ({
  ...
  fadeWindowStart: UNBLOCKED_BLOCK_UNTIL_TIMESTAMP,
  consecutiveBlocks: 0,
  fadedOrderHashes: undefined, // no block, no reason to name faded orders
}));
